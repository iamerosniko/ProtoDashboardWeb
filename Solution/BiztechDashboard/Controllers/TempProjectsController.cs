...
namespace BiztechDashboard.Controllers
{
    public class TempProjectsController : ApiController
    {
        ...
        public IHttpActionResult PostWDSB_TempProjects(List<WDSB_TempProjects> wDSB_TempProjects)
        {
            int good = 0;
            ...
            foreach (var temp in wDSB_TempProjects)
            {
                db.WDSB_TempProjects.Add(temp);
            ...
            return Ok("Good: "+ good +"Error: " + (wDSB_TempProjects.Count()-good).ToString());
        }
        //[ResponseType(typeof(WDSB_TempProjects))]
